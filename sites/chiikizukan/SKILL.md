...
| フィールド名 | 必須 | 種類 | 入力内容 |
|------------|------|------|--------|
| このプロジェクトでご自身の経験がどのように活かせそうですか？（200字以上） | 必須 | テキストエリア（200字以上） | COWORK_SKILL.md Step 4 のルールで生成した応募文。冒頭は my_profile.md の「名前」を使い「はじめまして、{名前}と申します。」から始め、my_profile.md の実績を1つ引用。地域課題解決への貢献意欲も1文入れる。「ぜひ一度、面談の機会をいただければ幸いです。」で締める |
